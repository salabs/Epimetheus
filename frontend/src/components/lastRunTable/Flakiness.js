...
const pickIcon = test_status => {
  ...
  switch (test_status) {
    case 'PASS':
      result = (
        <TestIcon text="Pass" type="check" iconColor={theme.colors.pass} />
      );
      break;
    case 'FAIL':
      result = (
        <TestIcon text="Fail" type="times" iconColor={theme.colors.fail} />
      );
      break;
    case 'SKIPPED':
      result = (
    ...
    case 'EMPTY':
      result = (
        <TestIcon text="Empty" type="minus" iconColor={theme.colors.skipped} />
      );
      break;
    default:
      result = '';
    ...
    slicedBuilds.length < 5
      ? flakinessTemplate
          ...
          .reduce((temp, templateItem) => {
            const indexOfBuilds = findIndex(slicedBuilds, templateItem);
            const buildNumbers = slicedBuilds.map(b => b.build_number);
