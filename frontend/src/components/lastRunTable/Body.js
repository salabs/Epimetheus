...
const Body = ({ id }) => {
  ...
  if (SUITE_FILTER_NESTED) {
    filteredRowData = filteredSuitesByBuildNumber
      .filter(element =>
        element.test_cases.some(
          test_cases =>
            test_cases.builds[0].test_status !== lastRunFilterPass.filterType
        )
      )
      .map(element => {
    ...
    filteredRowData = filteredSuitesByBuildNumber.filter(({ test_cases }) => {
      return test_cases.some(({ builds }) => {
        return builds
          .slice(0, 1)
          .some(
            ({ test_status }) => test_status !== lastRunFilterPass.filterType
          );
      });
    });
  }
  ...
  if (SUITE_FILTER_NESTED) {
    filteredRowData = filteredRowData
      .filter(element =>
        element.test_cases.some(
          test_cases =>
            test_cases.builds[0].test_status !== lastRunFilterFail.filterType
        )
      )
      .map(element => {
        return Object.assign({}, element, {
    ...
    filteredRowData = filteredRowData.filter(({ test_cases }) => {
      return test_cases.some(({ builds }) => {
        return builds
          .slice(0, 1)
          .some(
            ({ test_status }) => test_status !== lastRunFilterFail.filterType
          );
      });
    });
  }
