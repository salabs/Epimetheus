...
const Body = ({ id }) => {
    ...
    const firstBuildByBuildNumber = comparedDataState[0].filter(({ test_cases }) => {
        ...
        return {'full_name': test.full_name, 'status1': test.builds[0].test_status, 'status2': ''}
    });
    //Done twice since 2 builds
    const secondBuildByBuildNumber = comparedDataState[1].filter(({ test_cases }) => {
        return test_cases.some(({ builds }) => {
            return builds.some(
    ...
    secondBuildByBuildNumber.forEach(test_case2 => {
        firstBuildByBuildNumber.forEach(test_case1=>{
            if(test_case1.full_name === test_case2.full_name){
                matching_array.push({'full_name': test_case1.full_name, 'status1': test_case1.status1, 'status2': test_case2.status2} );
            }
        })
    })
    ...
    if(!compareFilterMatch.isChecked && !compareFilterMismatch.isChecked){
        combined_json = first_not_matching.concat(matching_array, second_not_matching)
    }else if(compareFilterMatch.isChecked && !compareFilterMismatch.isChecked){
